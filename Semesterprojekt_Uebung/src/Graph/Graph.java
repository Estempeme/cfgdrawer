...
public class Graph {
	...
	private String path = "C:/Users/Misi HP/Documents/Iskola/Humboldt/programok/Semesterprojekt_Uebung/graph.png";
	...
	public void addEdge(int from, int to) {
		...
		if (firstinst == null) { // There are no nodes yet.
			if (debugprint) System.out.println("Adding new edge "+to+" to new graph's new instr "+from);
			firstinst = new Instruction(from, to);
		} else {
			Instruction instr = searchInstr(from);
			if (instr == null) { // This node doesn't yet exist.
				if (debugprint) System.out.println("Adding new edge "+to+" to new instr "+from);
	...
	/*
	 * Search for the Instruction (node) with the ID id.
	 * Returns a pointer to it, if it exists. Otherways returns null.
	 */
	public Instruction searchInstr(int id) {
		//System.out.println("The ID you are looking for is: " + id);
		if (firstinst == null) return null;
	 ...
	 *  		to: the id of the instruction to which the edge leads
	 */
	...
	public void createPicture() {
		...
		while (inst != null) {
			...
			while (edge != null) {
				...
				String out = Integer.toString(id)+" -> "+edgeTo+" [label=\""+label+"\"];";

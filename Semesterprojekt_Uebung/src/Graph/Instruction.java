...
public class Instruction {
	...
	private Instruction next; // pointer to the next vertex in the list (has nothing to do with the graph)
	private Edge edges;
	private String type;
	...
	public Edge searchEdge(int id) {
		...
		Edge edge = edges;
		while (edge != null) {
			if (edge.getID() == id) return edge; // Edge found.
			if (edge.getID() > id) return null; // Id is too big: no such edge.
			edge = edge.getNext();
		}
